...
from whalu.analysis import add_timestamps, species_summary, hourly_activity, daily_counts
...
def cmd_mbari(args: argparse.Namespace) -> None:
    ...
    with _progress() as progress:
        for year, month in periods:
            console.print(Rule(f"[bold cyan]MBARI Pacific Sound  ·  {year}-{month:02d}", style="cyan"))
            ...
            log.info("Processing [bold cyan]%d[/bold cyan] files%s → [dim]%s[/dim]",
                     len(keys), suffix, args.output_dir)
            ...
            for key in keys:
                ...
                try:
                    ...
                    if limit_s is not None:
                        ...
                        audio, dur = mbari.download_audio(key, model.sample_rate, limit_s=limit_s)
                        log.info("[green]✓[/green] %-42s  [cyan]%.2fh[/cyan]", fname, dur / 3600)
                        ...
                        for chunk_audio, chunk_start_s, chunk_dur_s in mbari.stream_chunks(
                            key, model.sample_rate
                        ):
                            ...
                            chunk_df = run_detections(
                                model, chunk_audio, source_name=source_name, offset_s=chunk_start_s
                            )
                            chunk_dfs.append(chunk_df)
                            total_dur += chunk_dur_s
                            progress.advance(chunk_task)
                        progress.remove_task(chunk_task)
                        df = pl.concat(chunk_dfs) if chunk_dfs else pl.DataFrame({
                            "source": [], "time_start_s": [], "time_end_s": [],
                            "species": [], "confidence": [], "rank": [],
                        })
                        log.info("[green]✓[/green] %-42s  [cyan]%.1fh[/cyan]  %d detections",
                                 fname, total_dur / 3600, len(df))
    ...
    console.print(Panel(stats, title="Run complete", border_style="green", padding=(0, 2)))
...
def cmd_orcasound(args: argparse.Namespace) -> None:
    ...
    with _progress() as progress:
        ...
        log.info("Loaded [cyan]%.1fs[/cyan] at [cyan]%dHz[/cyan]", dur, model.sample_rate)
        df = run_detections(model, audio, source_name=f"orcasound/{Path(key).stem}")
        store.write(df, stem)
        progress.update(task, completed=1)

    log.info("[green]✓[/green] %d rows → [dim]%s/%s.parquet[/dim]", len(df), args.output_dir, stem)
    _render_summary(store)
...
def cmd_info(args: argparse.Namespace) -> None:
    ...
    if source_id and source_id not in SOURCE_REGISTRY:
        log.error("Unknown source '%s'. Available: %s", source_id, ", ".join(SOURCE_REGISTRY))
        sys.exit(1)

    sources = [SOURCE_REGISTRY[source_id]] if source_id else list(SOURCE_REGISTRY.values())
    for src in sources:
...
def _render_source_info(src: SourceInfo) -> None:
    ...
    console.print(RichColumns([
        Panel(facts,    title=f"[bold {_O}]Sensor & Data[/bold {_O}]",
              border_style=_S, padding=(1, 2), expand=False),
        Panel(sp_table, title=f"[bold {_O}]Species[/bold {_O}]",
              border_style=_S, padding=(1, 1), expand=False),
...
def cmd_analyze(args: argparse.Namespace) -> None:
    ...
    log.info("Loading %d Parquet files from [dim]%s[/dim]...", len(files), args.input_dir)
    df = pl.concat([pl.read_parquet(f) for f in files])
    df = add_timestamps(df)

    n_windows = df.filter(pl.col("rank") == 1).shape[0]
    n_detections = df.filter((pl.col("rank") == 1) & (pl.col("confidence") >= 0.5)).shape[0]
    ...
    sp_table = Table(
        box=box.ROUNDED, border_style=_S,
        header_style=f"bold white on {_S}",
        padding=(0, 1),
    )
    ...
    global_max = max(
        (hourly[sp].max() or 0.0)
        for sp in top_species if sp in hourly.columns
    ) or 1.0

    heat = Table(box=box.SIMPLE_HEAD, border_style=_D, padding=(0, 1),
                 header_style=f"bold {_O}")
    ...
    day_table = Table(box=box.SIMPLE_HEAD, border_style=_D, padding=(0, 1),
                      header_style=f"bold {_O}")
...
def _print_banner() -> None:
    ...
    _CMDS = [
        (True,  "whalu scan mbari",    "--start 2026-03",               "detect whales · MBARI Pacific Sound"),
        (False, "",                    "  --max-files 1 --limit-hours 1","↳ single-day test (~172 MB/file)"),
        (True,  "whalu scan mbari",    "--start 2023-07 --end 2023-10",  "scan a date range"),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    m = scan_sub.add_parser("mbari", help="MBARI Pacific Sound (s3://pacific-sound-16khz)")
    m.add_argument("--start", required=True, metavar="YYYY-MM")
    m.add_argument("--end", metavar="YYYY-MM",
                   help="End year-month inclusive (default: same as --start)")
    m.add_argument("--max-files", type=int, default=None, metavar="N",
                   help="Only process the first N files (default: all)")
    m.add_argument("--limit-hours", type=float, default=None, metavar="N",
    ...
    o.add_argument("--key", metavar="S3_KEY",
    ...
    az.add_argument("--input-dir", default="data/detections/mbari",
                    help="Directory of detection Parquet files")
    az.add_argument("--top-n", type=int, default=5,
